...
public class GameControllerTests {
    ...
    void testReadPlayer() throws Exception {
        ...
        // when & then:
        mvc.perform(get("/players/{playerId}", player.getPlayerId()))
                .andExpect(status().isOk())
    ...
    void testPlaceBet() throws Exception {
        ...
        // when & then:
        mvc.perform(post("/players/{playerId}/bets", player.getPlayerId())
        ...
        final List<GameRound> allRounds = gameRoundRepository.findByPlayerId(player.getPlayerId());
    ...
    void testReadGameRound() throws Exception {
        ...
        // when:
        mvc.perform(get("/game-rounds/{roundId}", gameRound.getId()))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(gameRound.getId()))
                .andExpect(jsonPath("$.bet.stake.formatted").value("€10.00"))
