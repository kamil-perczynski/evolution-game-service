...
public class BetOperations {
    ...
    public synchronized void tryPlaceBet(String playerId, Bet bet) {
        ...
        if (betPlacementCost.isGreater(player.getBalance())) {
        ...
        playerRepository.save(
                player.toBuilder()
                        .balance(player.getBalance().minus(betPlacementCost))
                        .build()
        );
    }

    private static Money calcBetPlacementCost(Bet bet, Player player) {
        if (player.isFreeRoundAvailable() || bet.getMode() == BetMode.PLAY_FOR_FREE) {
            return Money.ZERO;
        }
