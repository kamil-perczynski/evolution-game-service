...
public class GameRoundMachine {
    ...
    private Money drawMoneyWin(Bet bet) {
        ...
        return bet.getStake().multiply(multiplier);
    }

    private int drawPayoutMultiplier() {
        if (randomFacts.drawBigWin()) {
            return gameRoundMachineProperties.getBigWinMultiplier();
        }
        if (randomFacts.drawMediumWin()) {
            return gameRoundMachineProperties.getMediumWinMultiplier();
        }
        return gameRoundMachineProperties.getSmallWinMultiplier();
    }
