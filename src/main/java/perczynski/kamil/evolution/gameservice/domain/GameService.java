...
public class GameService {
    ...
    private synchronized Player applyWins(String playerId, Bet bet, Win win) {
        ...
        if (bet.getMode() == BetMode.PLAY_FOR_FREE) {
            return mostRecentPlayer;
        }

        final Money nextBalance = mostRecentPlayer.getBalance().plus(win.getPayout());

        final Player nextPlayer = mostRecentPlayer.toBuilder()
                .freeRoundAvailable(win.isFreeRound())
                .balance(nextBalance)
                .build();
    ...
    private static GameRound toBetOutcome(String playerId,
        ...
        return GameRound.builder()
                ...
                .bet(bet)
                .win(win)
                .nextBalance(nextPlayer.getBalance())
                .build();
    }

    private static void checkBetAmount(Bet bet) {
        if (bet.getStake().getAmount() < 1_00 || bet.getStake().getAmount() > 10_00) {
